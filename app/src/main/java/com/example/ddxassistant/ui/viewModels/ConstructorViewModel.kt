...
class ConstructorViewModel(private val workoutInteractor: WorkoutInteractor): ViewModel() {
    ...
    fun getScreenStateLiveData(): LiveData<WorkoutScheduleStates> = screenStateLiveData
    ...
    fun writeToList(exercise: Exercise){
        workoutInteractor.addExerciseToList(exercise)
    }
    fun getExerciseList(): List<Exercise>{
        return workoutInteractor.getCurrentWorkoutList()
    }
}